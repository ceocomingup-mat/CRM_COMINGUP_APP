    ...
    <div className="min-h-svh bg-slate-50">
      <aside className="fixed inset-y-0 left-0 flex w-60 flex-col border-r border-slate-200 bg-white">
        <div className="flex items-center gap-3 px-5 py-4">
          <div className="grid h-9 w-9 place-items-center rounded-lg bg-violet-600 font-bold text-white">
            C
          </div>
          <span className="font-semibold text-slate-900">ComingUP CRM</span>
        </div>
        ...
        <div className="border-t border-slate-200 px-4 py-3">
          <div className="truncate text-sm font-medium text-slate-900">{fullName}</div>
          <div className="text-xs text-slate-500">
            {ROLE_LABEL[profile.role] ?? profile.role}
          </div>
          <button
            onClick={logout}
            className="mt-2 w-full rounded-lg border border-slate-300 px-3 py-1.5 text-sm text-slate-700 transition hover:bg-slate-100"
          >
            Wyloguj
          </button>
        </div>
